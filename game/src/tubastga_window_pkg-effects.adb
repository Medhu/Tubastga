   ...
   begin
      ...
      while Piece.Client_Piece.Pieces_Client_List.Has_Element (Trav_Pieces) loop
         ...
         while Effect.Effect_List.Has_Element (Trav_Effects) loop
            ...
            if An_Effect.Effect_Name = Tubastga_Game.Effect_Action_Point then
               A_Piece.all.Action_Point := An_Effect.Aux;

            elsif An_Effect.Effect_Name = Tubastga_Game.Effect_Slot_1 then
               A_Piece.all.Storage.Slots (1) := Goods.Aux_To_Goods_Info (An_Effect.Aux);

            elsif An_Effect.Effect_Name = Tubastga_Game.Effect_Slot_2 then
               A_Piece.all.Storage.Slots (2) := Goods.Aux_To_Goods_Info (An_Effect.Aux);

            elsif An_Effect.Effect_Name = Tubastga_Game.Effect_Slot_3 then
               A_Piece.all.Storage.Slots (3) := Goods.Aux_To_Goods_Info (An_Effect.Aux);

            elsif An_Effect.Effect_Name = Tubastga_Game.Effect_Stops then

               A_Piece.all.Stops (1) :=
                 Piece.Type_Piece_Id (Tubastga_Game.Carrier.Get_Tower_Id (1, An_Effect.Aux));
               A_Piece.all.Stops (2) :=
                 Piece.Type_Piece_Id (Tubastga_Game.Carrier.Get_Tower_Id (2, An_Effect.Aux));
               A_Piece.all.Stops (3) :=
                 Piece.Type_Piece_Id (Tubastga_Game.Carrier.Get_Tower_Id (3, An_Effect.Aux));

            elsif An_Effect.Effect_Name = Tubastga_Game.Effect_Load then
               A_Piece.all.Load (1) := Tubastga_Game.Carrier.Get_Tower_Goods_Id (1, An_Effect.Aux);
               A_Piece.all.Load (2) := Tubastga_Game.Carrier.Get_Tower_Goods_Id (2, An_Effect.Aux);
               A_Piece.all.Load (3) := Tubastga_Game.Carrier.Get_Tower_Goods_Id (3, An_Effect.Aux);

            elsif An_Effect.Effect_Name = Tubastga_Game.Effect_Unload then
               A_Piece.all.Unload (1) :=
                 Tubastga_Game.Carrier.Get_Tower_Goods_Id (1, An_Effect.Aux);
               A_Piece.all.Unload (2) :=
                 Tubastga_Game.Carrier.Get_Tower_Goods_Id (2, An_Effect.Aux);
               A_Piece.all.Unload (3) :=
                 Tubastga_Game.Carrier.Get_Tower_Goods_Id (3, An_Effect.Aux);

            elsif An_Effect.Effect_Name = Tubastga_Game.Effect_Captain then
